...
| **测试**   | **pytest** + **httpx** (TestClient)           | 289+ 个单元 + 集成 + 基准测试 |
| **包管理** | **uv**                                       | Python 依赖管理                                       |
...
项目包含 **289 个测试**（单元测试 + 集成测试 + 延迟基准测试），覆盖安全模块、数据模型、工具函数、数据库层、SchemaSync、账号模块、课程模块、自动签到模块和认证路由：
